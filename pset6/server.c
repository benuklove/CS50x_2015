...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char *sp2p;
            sp1p = strchr(line, sp);
            sp2p = strrchr(line, sp);
            if (sp1p == NULL || sp2p == NULL || sp1p == sp2p)
            {
                printf("\033[31m");
            ...
            int rqtarget0 = sp1p - line + 1;
            int rqtarget1 = sp2p - line - 1;
            int rqtargetsize = rqtarget1 - rqtarget0 + 1;
            char reqtarget[rqtargetsize + 1];
            int rtindex;
            for (rtindex = 0; rtindex < rqtargetsize; rtindex++)
            {
                reqtarget[rtindex] = line[rqtarget0 + rtindex];
            }
            reqtarget[rqtargetsize] = '\0';
            int quote = '"';
            char *qp;
            if ((qp = strchr(reqtarget, quote)))
            {
                printf("\033[31m");
                printf("400 Bad Request\n");
                printf("\033[39m\n");
            ...
            int qmark = '?';
            char *qmarkp;
            qmarkp = strchr(reqtarget, qmark);
            int abspathsize;
            int querysize; 
            if (qmarkp)
            {
                // size of abspath includes question mark
                abspathsize = qmarkp - reqtarget;
                querysize = rqtargetsize - abspathsize;
            }
            else
            {
                abspathsize = rqtargetsize;
                querysize = 0;
            }
            char abspath[abspathsize + 1];
            int apindex;
            for (apindex = 0; apindex < abspathsize; apindex++)
            {
                abspath[apindex] = reqtarget[apindex];
            }
            abspath[abspathsize] = '\0';
            
            char query[querysize + 1];
            int qindex;
            for (qindex = 0; qindex < querysize; qindex++)
            {
                query[qindex] = reqtarget[abspathsize + 1 + qindex];
            }
            query[querysize] = '\0';
            ...
            char path[50];
            strcpy(path, root);
            strncat(path, abspath, abspathsize);
            ...
            int peri = '.';
            char *ppp;
            ppp = strchr(abspath, peri);
            int periloc = ppp - abspath;
            int extsize = abspathsize - periloc;
            char extension[extsize];
            int exindex;
            for (exindex = periloc + 1; exindex < abspathsize; exindex++)
            {
                extension[exindex - periloc - 1] = abspath[exindex];
            }
...
// convert extension characters to all lowercase
int lower(char extchar)
{
    if (isupper(extchar))
    {
        extchar = extchar + 32;
...
const char* lookup(const char* extension)
{
    ...
    for (i = 0; i < len; i++)
    {
        char extchar = lower(extension[i]);
        word[i] = extchar;
    }
    int r0 = strncmp(word, css, len);
    int r1 = strncmp(word, html, len);
    int r2 = strncmp(word, gif, len);
    int r3 = strncmp(word, ico, len);
    int r4 = strncmp(word, jpg, len);
    int r5 = strncmp(word, js, len);
    int r6 = strncmp(word, png, len);
